...
for im=1:4
    for ie=1:5
        for iw=1:3
            ...
            X = X_init * [ones(1, n), zeros(1, n); zeros(1, n), ones(1, n)] + eps_0 * randn(m, 2 * n);
            X = zscore(X);
            X_test = randn(m_test, 2);
            y_test = generate_single_logistic(X_test, w);
            X_test = X_test * [ones(1, n), zeros(1, n); zeros(1, n), ones(1, n)] + eps_0 * randn(m_test, 2 * n);
            ...
            AUC_best(im, ie, iw) = test_single_logistic(X_test, y_test, ones(2 * n, 1) / (2 * n));
            %[~, idx_max] = max(w_est);% does not matter for equal noise, but does for varyings
            [~, w_single, ~, ~] = maximize_evidence_single_logistic_laplace(X(:, [1, n+1]), y);
            AUC_single(im, ie, iw) = test_single_logistic(X_test(:, [1, n+1]), y_test, w_single);


            num_point_evidence = sum(w_est)^2 / (sum(w_est .* w_est) + 1e-10);
            X_lin = zscore(X(1:m, :) * w_est);
            diff_ev = X_lin - zscore(X_init * ones(2, 1));
            ...
            fig_name = strcat('figures\combine_clique_w_second_', num2str(w_all(iw)), '_eps_',...
            ...
            diff = X_combo_lin - zscore(X_init * ones(2, 1));
...
for iw=1:size(w_all, 2)
    ...
    fig_name = strcat('figures\auc_best_combo_diff_second_', num2str(w_all(iw)));
    saveas(h, strcat(fig_name, '.png'), 'png');
    saveas(h, strcat(fig_name, '.eps'), 'psc2');
    ...
    fig_name = strcat('figures\auc_combo_evidence_diff_second_', num2str(w_all(iw)));
    ...
    fig_name = strcat('figures\auc_combo_single_diff_second_', num2str(w_all(iw)));
    ...
    fig_name = strcat('figures\signal_fraction_best_combo_diff_second_', num2str(w_all(iw)));
    ...
    fig_name = strcat('figures\signal_fraction_combo_evidence_diff_second_', num2str(w_all(iw)));
    ...
    fig_name = strcat('figures\signal_fraction_combo_single_diff_second_', num2str(w_all(iw)));
