...
[w_mix, pi, hessian_mix, A_mix, ~] = learn_optimal_mixture_logistic(X, y, K, 1.);
...
[w_mix_adeq, pi_adeq, hessian_mix_adeq, A_mix_adeq] = get_adequate_mixture_model(X, y, size(pi, 1), 1., alpha, pi, w_mix, A_mix, hessian_mix);
...
for index=1:size(K_all, 2)
    ...
    for iter=1:max_iter
       ...
       % Sparse mixture
       if (index == 1)
           [w_mix, pi, hessian_mix, A_mix, ~] = learn_optimal_mixture_logistic(X, y, 200, 1e-6);
           [w_mix, pi, hessian_mix, A_mix] = filter_mixture(w_mix, pi, hessian_mix, A_mix);
